...
@WebServlet("/enterPinPinChangeServlet")
public class EnterPinPinChangeServlet extends HttpServlet{
	/**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		int pinMoneyTransfer = Integer.parseInt(req.getParameter("withpin"));
		...
		String invalidPinLockString = "invalidpinlock";
			...
			if (userpinMoneyTransfer > 0) {
				if (userpinMoneyTransfer == pinMoneyTransfer) {
					res.sendRedirect("pinChange.jsp");
				} else {
